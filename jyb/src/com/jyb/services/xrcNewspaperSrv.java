...
@Service("xrcNewspaperSrv")
public class xrcNewspaperSrv {
	...
	public List<Map<String,Object>> selectEditionnum() throws Exception{
		List<Map<String,Object>> list = (List<Map<String,Object>>)ds.findList("xrcNewspaperMapper.selectEditionnum");
		return list;
	}
	...
	public List<Map<String,Object>> queryEditionnum(String editionnum) throws Exception{
		List<Map<String,Object>> list = (List<Map<String,Object>>)ds.findList("xrcNewspaperMapper.queryEditionnum",editionnum);
		return list;
	}
	...
	public List<Map<String,Object>> queryPagenum(String newspaperid,String pagenum) throws Exception{
		...
		List<Map<String,Object>> list = (List<Map<String,Object>>)ds.findList("xrcNewspaperMapper.queryPagenum",map);
		return list;
	}
	...
	public Map selectNespaperPart(String editionnum,String pagenum) throws Exception{
		List<Map> list = (List<Map>)ds.findList("xrcNewspaperMapper.queryEditionnum",editionnum);
		Map map = null;
		if(list!=null && !list.isEmpty()){
			map = list.get(0);
			map.put("pagenum", pagenum);
			list = (List<Map>)ds.findList("xrcNewspaperMapper.queryPagenum",map);
			if(list!=null && !list.isEmpty()){
				map = list.get(0);
				String imgname = (String)map.get("imgname");
				list = (List<Map>)ds.findList("xrcNewspaperMapper.queryPart",map);
				map = new HashMap();
				map.put("result", 1);
				map.put("list", list);
	...
	public Map toNewspaper(String editionnum,String pagenum,String partid) throws Exception{
		...
		Map map = null;
		if(partid!=null){
			list = (List<Map<String,Object>>)ds.findList("xrcNewspaperMapper.queryEditionPagenum",partid);
			map = list.get(0);
			editionnum = (String)map.get("editionnum");
			pagenum = (String)map.get("pagenum");
		}
		Map reMap = new HashMap();
		list = (List<Map<String,Object>>)ds.findList("xrcNewspaperMapper.toNewspaperEditionnum");
		if(list!=null && !list.isEmpty()){
			...
			list = (List<Map<String,Object>>)ds.findList("xrcNewspaperMapper.toNewspaperPagenum",map);
			if(list!=null && !list.isEmpty()){
				...
				list = (List<Map<String,Object>>)ds.findList("xrcNewspaperMapper.queryPartSynopsis", map);
				if(list!=null && !list.isEmpty()){
	...
	public List showNewspaper(Map map) throws Exception{
		...
		List list = (List)ds.findList("xrcNewspaperMapper.showNewspaper",map);
	...
	public Map selectNewspaperPart(String partid) throws Exception{
		List<Map> list = (List<Map>)ds.findList("xrcNewspaperMapper.queryPartContent",partid);
		Map map = list.get(0);
		return map;
	}
	...
	public void updatePartHits(String partid) throws Exception{
		ds.findList("xrcNewspaperMapper.updatePartHits",partid);
	}
	...
	public Map updateNewspaperPart(String partid,String parttitle,String content,String aid) throws Exception{
		...
		int result = ds.update("xrcNewspaperMapper.updateNewspaperPart", map);
		map.clear();
	...
	public void insertNewspaperEdition(List<Map> list,Map msg) throws Exception{
		...
		if("1".equals(msg.get("pagenum"))){
			ds.update("xrcNewspaperMapper.updateNewspaperFirstimg", msg);
		}
		String uuid = UUIDUtil.getUUID();
		msg.put("editionid", uuid);
		ds.save("xrcNewspaperMapper.insertNewspaperEdition", msg);
		for(Map map:list){
			...
			ds.save("xrcNewspaperMapper.insertNewspaperPart", map);
		}
	}
	...
	public void insertNewspaper(String editionnum) throws Exception{
		...
		ds.save("xrcNewspaperMapper.insertNewspaper", map);
	}
	...
	public Map updateNewspaperEdition(String editionnum,String pagenum,String pagetitle) throws Exception{
		...
		boolean test = false;
		List<Map<String,Object>> list = (List<Map<String, Object>>) ds.findList("xrcNewspaperMapper.toNewspaperEditionnum");
		if(list!=null && !list.isEmpty()){
			if(editionnum!=null && !"".equals(editionnum)){
				for(Map tMap:list){
					if(tMap.get("editionnum").equals(editionnum)){
			...
			if(test){
				test = false;
				list = (List) ds.findList("xrcNewspaperMapper.toNewspaperPagenum",map);
				if(list!=null && !list.isEmpty()){
					if(pagenum!=null && !"".equals(pagenum)){
						for(Map tMap:list){
							if(tMap.get("pagenum").equals(pagenum)){
				...
				if(test && pagetitle!=null && !"".equals(pagetitle)){
					map.put("result", 1);
					map.put("pagetitle", pagetitle);
					ds.update("xrcNewspaperMapper.updateNewspaperEdition", map);
				}else{
